...
function initTypingEffect(prefersReducedMotion) {
  ...
  // Keep static text and skip animation for reduced motion users
...
function initMobileMenu() {
  ...
  function openMenu() {
    ...
    // A <dialog> focuses its first focusable child by default.
    // Move focus to the dialog itself so "Home" is not visually
    // highlighted every time.
    menu.focus({
      preventScroll: true
    });
  }
...
function initCustomCursor() {
  ...
  function followLoop() {
    ...
    // Stop the loop if the ring has practically caught up
    // to the pointer.
    if (
      Math.abs(dx) < 0.1 &&
      Math.abs(dy) < 0.1
